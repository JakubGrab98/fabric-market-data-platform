...
except `dim_spolka`, which reads directly from
...
## fact_ceny / fact_fundamenty / fact_makro

All three are near-identical in mechanism: Silver already produces (or, for
`fact_makro`, nearly produces) the exact Gold-shape columns specified in
...
(`fact_makro` drops Silver's `variable_id`, kept there only for lineage).

Idempotent upsert keys: `fact_ceny` on `(ticker, date)`; `fact_fundamenty`
on `(ticker, period_end_date, statement_type, metric_name)`; `fact_makro` on
...
- Backfilling `dim_data`'s range choice against real data volume — start
