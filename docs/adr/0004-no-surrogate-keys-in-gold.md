...
keys (e.g. an auto-incrementing `company_id`) rather than natural business
...
## Decision
Gold tables use natural keys throughout: `dim_company` is keyed by `ticker`,
`dim_date` by `date`, and each fact table's grain is expressed directly in
terms of those natural keys (e.g. `fact_prices` on `(ticker, date)`). No
...
- No SCD history tracking is possible without revisiting this (e.g. if a
  ...
  recoverable from `dim_company` alone). No such requirement exists today.
